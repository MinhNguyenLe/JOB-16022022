...
export default class Slot {
  ...
  private nameList: { name: string; image: string }[];
   ...
   */
  set names(names: { name: string; image: string }[]) {
    this.nameList = names;
  ...
  get names(): { name: string; image: string }[] {
  ...
  init() {
    ...
    let randomNames = Slot.shuffleNames<{ name: string; image: string }>(
      this.nameList
    );
    ...
    randomNames.forEach((item) => {
      ...
      newItemImage.alt = item.image;
      newItemImage.classList.add("image");
      newReelItem.classList.add("item");
      newReelItem.appendChild(newItemImage);
      newReelItem.appendChild(newItemName);
  ...
  public async spin(): Promise<boolean> {
    if (!this.nameList.length) {
      console.error("Item List is empty. Cannot start spinning.");
      return false;
    }
    ...
    const winner = randomNames[randomNames.length - 3];
    ...
    Array.from(reelContainer.children)
      .slice(0, reelContainer.children.length - 5)
    ...
    const luckyName = document.getElementById(
      "lucky-name"
    ) as HTMLDivElement | null;
    const luckyImg = document.getElementById(
      "lucky-image"
    ...
    if (luckyName) {
      ...
      small.innerHTML = "♦";
      luckyName.innerHTML = "";
      luckyName.appendChild(small.cloneNode(true));
      luckyName.append(" ♦ ");
      luckyName.append(winner.name);
      luckyName.append(" ♦ ");
      luckyName.appendChild(small.cloneNode(true));
    }
    if (luckyImg) {
      luckyImg.src = winner.image;
      luckyImg.alt = winner.image;
    }
